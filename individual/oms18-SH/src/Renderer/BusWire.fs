...
let wire (wModel: Model) (wId: CommonTypes.ConnectionId): Wire = failwithf "Not impelmented"
...
type Error = { Msg: string; Pos: XYPos }
...
let getErrors (model: Model): Error list =
    ...
    |> List.collect
        (fun w ->
            let srcPortType = Symbol.portType model.Symbol w.SrcPort

            let targetPortType =
                Symbol.portType model.Symbol w.TargetPort

            if srcPortType = targetPortType
               && srcPortType = CommonTypes.PortType.Output then
...
let init n () =
    ...
    let (ipLst, opLst) = Symbol.getAllPorts symbols
    ...
    let makeRandomWire () =
        ...
        let p1, p2 =
            ipLst.[rng.Next(0, n - 1)], opLst.[rng.Next(0, n - 1)]
...
let update (msg: Msg) (model: Model): Model * Cmd<Msg> =
    ...
    | Symbol sMsg ->
        let wires =
            ...
            | Symbol.DeleteSymbols sIdLst ->
                let pIdSet =
                    Set.ofList
                    <| Symbol.getPortsFromSymbols model.Symbol sIdLst
    ...
    | Delete wId ->
        let wires =
            model.WX |> List.filter (fun w -> w.Id <> wId)
        ...
        let wires =
            ...
            |> List.map
                (fun w ->
                    if w.Id = wId then
                        let start =
                            match w.DragPositions with
                            | Some (start, _) -> start
                            | None -> failwithf "Called Dragging before StartDrag!"

                        { w with
                              DragPositions = Some(start, p) }
                    else
                        w)

...
/// A type alias that holds a tuple of the two submodels
type SubModel = BusWire.Model * Symbol.Model

/// CreateElements holds a list over each symbol and wire that is to be created.
type CreateElements =
    {
...
/// DragState holds the current selection
type SelectionState =
...
/// This contains the data currently copied, as well as how many times this data
/// has been pasted to be able to give each a different suffix each time.
type CopyState =
